...
const Skills = ({ skillData }: Props) => {
    ...
    return (
        <SectionWrapper id='skills' className="min-h-screen mt-12 md:mt-0 mx-4 md:mx-0 xl:my-20 2xl:my-0">
            ...
            <div className="md:w-1/2 lg:w-1/3 mx-auto mt-6 bg-white dark:bg-grey-800 p-2 flex flex-nowrap items-center gap-3 overflow-x-auto scroll-hide rounded-md">
                {categories.map((c: string, i: number) => (
                    <span
                        key={i}
                        onClick={() => setCategory(c)}
                        className={`p-1.5 md:p-2 text-sm md:text-base whitespace-nowrap flex-shrink-0 text-center cursor-pointer rounded-md ${category.toLowerCase() === c.toLowerCase()
